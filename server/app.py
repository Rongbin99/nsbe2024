...
def newpost():
    conn = sqlite3.connect("data/app.db")
    conn.execute("INSERT INTO posts VALUES (?, ?);", (request.args.get('id'), request.args.get('image')))
    conn.commit()
...
def newimage():
    conn = sqlite3.connect("data/app.db")
    conn.execute("INSERT INTO posts VALUES (?, ?);", (request.args.get('id'), request.args.get('data')))
    conn.commit()
